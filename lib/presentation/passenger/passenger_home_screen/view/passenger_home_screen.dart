...
class _PassengerHomeScreenState extends State<PassengerHomeScreen> {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      Provider.of<PassengerHomeScreenController>(context, listen: false).getRoutesList();
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    final userHomeScreenState = Provider.of<PassengerHomeScreenController>(context);
    return Scaffold(
        appBar: AppBar(
            title: Text(
          "",
                ...
                child: Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      InkWell(
                        onTap: () {
                          Provider.of<PassengerHomeScreenController>(context, listen: false).getSearchResult(
                        ...
                        child: Padding(
                          ...
                          child: Container(
                            ...
                            decoration:
                                BoxDecoration(color: ColorConstants.mainBlue, borderRadius: BorderRadius.circular(10)),
                            padding: EdgeInsets.symmetric(vertical: 10),
                            child: Center(
                                child: Text(
                              "Search",
                              style:
                                  TextStyle(color: ColorConstants.mainWhite, fontWeight: FontWeight.bold, fontSize: 18),
                            )),
                          ),
                        ),
                      ),
                              ...
                              child: SingleChildScrollView(
                                child: Column(
                                  children: List.generate(
                                      userHomeScreenState.searchResultResModel?.buses?.length ?? 0,
                                      (index) => PassengerHomeScreenBussesCard(
                                            routeId: userHomeScreenState.searchResultResModel?.buses?[index]
                                                    .routeAssignments?[0].routeId // Todo: route id need to be changed
                                                    .toString() ??
                                                "",
                                            imagePath: userHomeScreenState.searchResultResModel?.buses?[index].image
                                                    .toString() ??
                                                "",
                                            name: userHomeScreenState.searchResultResModel?.buses?[index].name
                                                    .toString() ??
                                                "",
                                            busNumber: userHomeScreenState
                                                    .searchResultResModel?.buses?[index].numberPlate
                                                    .toString() ??
                                                "",
                                            busRoute: userHomeScreenState.searchResultResModel?.buses?[index].isActive
                                                    .toString() ??
                                                "",
                                          )),
                                ),
                              ...
                              onTap: () {
                                Navigator.of(context).pushReplacement(MaterialPageRoute(
                                    builder: (context) => RouteDetailsScreen(
                                          routeId: userHomeScreenState.routesListResModel?.routesList?[index].id ?? "",
                                        )));
                              },
                              child: Container(
                                decoration: BoxDecoration(
